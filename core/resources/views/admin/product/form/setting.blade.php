...
@section('panel')
    <div class="row gy-4">
        ...
        <div class="col-xxl-10 col-lg-9 col-md-8 col-sm-9 order-2">
            <form action="{{ route('admin.products.store', @$product->id) }}" id="productForm" method="POST"
                    ...
                    <div class="col-xxl-8 col-xl-8">
                        ...
                        <div class="tab-content" id="v-pills-tabContent">
                            ...
                            <div class="tab-pane fade" id="media-content" role="tabpanel"
                                aria-labelledby="media-content-tab">
                            ...
                            <div class="tab-pane fade show" id="extra-description" role="tabpanel"
                                aria-labelledby="extra-description-tab">
                                @include('admin.product.partials.extra_description')
                            </div>

                            <div class="tab-pane fade" id="downloadable-config" role="tabpanel"
                                aria-labelledby="downloadable-config-tab">
...
@if (Route::is('admin.products.edit'))
    @push('breadcrumb-plugins')
        <a href="{{ route('product.detail', $product->slug) }}" target="blank"
            class="btn btn--primary view-in-shop">@lang('View In Shop')</a>
    @endpush
@endif
        ...
        (function($) {
            ...
            $('#receiveForm').on('submit', function(e) {
                ...
                $.post("{{ route('admin.stock.receive') }}",
                    ...
                    notify('error', err.responseJSON?.message || 'Error');
                });
            });

            $('#purchaserSelect').on('change', function() {
                const isNew = $(this).val() === 'new';
                $('#newPurchaserInput').toggleClass('d-none', !isNew)
                    .prop('required', isNew);
            });
            ...
            const handleDescriptionTabShow = () => {
                ...
                if (nicEditorInstanceForDescription.nicInstances.filter((element) => element.e.id == textareaField)
                    .length == 0) {
            ...
            const formatAttributes = (state) => {
                ...
                if (data.type === 'color') {
                    formattedAttribute = $(
                        `<div><span style="background-color: #${data.value}">&nbsp;</span>${state.text}</div>`);
            ...
            const buildAttributeField = (data, animate = true) => {
                ...
                if (productAttributeValues[data.value]) {
                    $(attributesValueWrapper).find(`#attribute${data.value}`).val(productAttributeValues[data
                        .value])
                }

                $(attributesValueWrapper).find(`#attribute${data.value}`).select2({
                    dropdownParent: $(attributesValueWrapper).find(`#attribute${data.value}`).parent(
                        '.select2-parent'),
                    closeOnSelect: false,
