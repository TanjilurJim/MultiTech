@php
    ...
    /**
     * Can the current admin see this item?
     * Accepts   'perm.one'  OR  'perm.one|perm.two'
     */
    $allowed = function (?string $perm): bool {
        if (!$perm) {
            return true;
        ...
        <div class="sidebar__menu-wrapper">
            <ul class="sidebar__menu">
                @foreach ($sideBarLinks as $key => $data)
                    @continue(!$allowed($data->permission ?? null))
                    @if (@$data->header)
                            ...
                            <div class="sidebar-submenu {{ menuActive(@$data->menu_active, 2) }} ">
                                <ul>
                                    @foreach ($data->submenu as $menu)
                                        ...
                                        @endphp
                                        @continue(!$allowed($menu->permission ?? null))
