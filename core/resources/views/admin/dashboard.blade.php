            ...
            <div class="card h-100">
                <div class="card-body">
                    ...
                    <div class="row gy-3 counting-widget">
                        <div class="col-12">
                            <x-widget value="{{ $deposit['total_deposit_pending'] }}" title="Pending Payment" style="2"
                                bg="white" color="danger" icon="las la-money-bill-wave"
                                link="{{ route('admin.deposit.pending') }}" icon_style="solid" overlay_icon=0 />
                        </div>
                        <div class="col-12">
                            <x-widget value="{{ $widget['pending_tickets'] }}" title="Pending Tickets" style="2"
                                bg="white" color="warning" icon="las la-ticket-alt"
                                link="{{ route('admin.ticket.pending') }}" icon_style="solid" overlay_icon=0 />
                        </div>
                        <div class="col-12">
                            <x-widget value="{{ $widget['low_stock_products'] }}" title="Low Stock Product" style="2"
                                bg="white" color="brown" icon="las la-layer-group"
                                link="{{ route('admin.products.low.stock') }}" icon_style="solid" overlay_icon=0 />
                        </div>
                        <div class="col-12">
                            <x-widget value="{{ $widget['out_of_stock_products'] }}" title="Out Of Stock Product"
                                style="2" bg="white" color="red" icon="lab la-dropbox"
                                link="{{ route('admin.products.out.of.stock') }}" icon_style="solid" overlay_icon=0 />
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="row g-3 account-widget">
                        ...
                        <div class="col-sm-6">
                            <x-widget value="{{ $widget['total_users'] }}" title="Total Registered" style="2"
                                bg="white" color="info" icon="la la-users" link="{{ route('admin.users.all') }}"
                                icon_style="solid" overlay_icon=0 />
                        </div>

                        <div class="col-sm-6">
                            <x-widget value="{{ $widget['profile_completed'] }}" title="Profile Completed" style="2"
                                bg="white" color="success" icon="la la-user-check"
                                link="{{ route('admin.users.profile.completed') }}" icon_style="solid" overlay_icon=0 />
                        </div>

                        <div class="col-sm-6">
                            <x-widget value="{{ $widget['active_users'] }}" title="Active" style="2" bg="white"
                                color="green" icon="la la-user-check" link="{{ route('admin.users.active') }}"
                                icon_style="solid" overlay_icon=0 />
                        </div>

                        <div class="col-sm-6">
                            <x-widget value="{{ $widget['banned_users'] }}" title="Banned" style="2"
                                bg="white" color="danger" icon="la la-user-slash"
                                link="{{ route('admin.users.banned') }}" icon_style="solid" overlay_icon=0 />
                        </div>

                        <div class="col-sm-6">
                            <x-widget value="{{ $widget['email_unverified_users'] }}" title="Email Unverified"
                                style="2" bg="white" color="5" icon="la la-envelope"
                                link="{{ route('admin.users.email.unverified') }}" icon_style="solid" overlay_icon=0 />
                        </div>

                        <div class="col-sm-6">
                            <x-widget value="{{ $widget['mobile_unverified_users'] }}" title="Mobile Unverified"
                                style="2" bg="white" color="2" icon="la la-mobile"
                                link="{{ route('admin.users.mobile.unverified') }}" icon_style="solid" overlay_icon=0 />
                        </div>
                    </div>
                </div>
            </div>
                                        ...
                                        @forelse($recentOrders as $order)
                                            <tr>
                                                <td>
                                                    @if ($order->user)
                                                        <a
                                                            href="{{ route('admin.users.detail', @$order->user->id) }}">{{ $order->user->fullname }}</a>
                                                    @else
                                                        @php $shippingAddress = json_decode($order->shipping_address) @endphp
                                                        <div>
                                                            <strong>{{ $shippingAddress->name ?? ($order->guest_name ?? 'Guest User') }}</strong><br>
                                                            {{ $shippingAddress->phone ?? 'N/A' }}<br>
