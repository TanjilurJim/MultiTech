...
Route::middleware('auth')->name('user.')->group(function () {
    ...
    Route::middleware(['check.status', 'registration.complete'])->group(function () {
        ...
        Route::namespace('User')->group(function () {
            ...
            // Route::get('wallet/add-moeny', [WalletController::class, 'addmoeny'])->name('addmoeny');
            // Route::post('wallet/add-moeny', [WalletController::class, 'addmoeny_store'])->name('addmoeny.store');
            // wallet ------------------------------------
            // wallet ------------------------------------
            // wallet ------------------------------------
...
Route::middleware('checkout.access')->name('user.')->group(function () {

    Route::name('checkout.')->group(function () {

        Route::controller('User\CheckoutController')->group(function () {
            Route::get('checkout/shipping-info', 'shippingInfo')
                ->name('shipping.info')
                ->middleware('checkout.step:shipping_info');

            Route::post('add-shipping-address', 'addShippingInfo')
                ->name('shipping.info.add')
                ->middleware('checkout.step:shipping_info');

            Route::get('checkout/delivery-methods', 'deliveryMethods')
                ->name('delivery.methods')
                ->middleware('checkout.step:delivery_method');

            Route::post('add-delivery-method', 'addDeliveryMethod')
                ->name('delivery.method.add')
                ->middleware('checkout.step:delivery_method');

            Route::get('order-confirmation/{order}', 'confirmation')->name('confirmation');
        });

        Route::controller('User\PaymentController')->group(function () {
            Route::get('checkout/payment-methods', 'paymentMethods')
                ->name('payment.methods')
                ->middleware('checkout.step:payment');
